...
// Importar a função de simular compra
...
export default function PromptPage({ params }: PromptPageProps) {
  ...
  // Adicionar estado para verificar compra
  ...
  useEffect(() => {
    ...
    const loadPrompt = () => {
      const foundPrompt = getPromptBySlug(params.category, params.slug)
      if (foundPrompt) {
    ...
    <div className="min-h-screen bg-black">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <nav className="flex items-center space-x-2 text-sm text-gray-400 mb-8">
          ...
          <span>/</span>
          <Link href={`/prompts/${params.category}`} className="hover:text-white capitalize">
            {params.category}
          </Link>
          <span>/</span>
            ...
            <Card className="bg-gray-900 border-gray-700">
              <CardContent className="p-6">
                <div className="text-center space-y-4">
                  ...
                  {/* Atualizar o botão de compra para simular compra em prompts gratuitos */}
                  {/* Na seção de compra/download, substituir o botão por: */}
                  <Button
                    onClick={() => {
                      if (prompt.isFree && user) {
                        // Simular "download" que conta como compra para prompts gratuitos
                        simulatePurchase(prompt.id, user.id)
                        alert("Download realizado! Agora você pode avaliar este prompt.")
                      } else if (!prompt.isFree) {
                        // Aqui seria o fluxo de pagamento real
